...
void leftright(char *setofcommands, char *homedirofshell, char *prevdir, struct list *head, struct log *top, struct log *acti)
{
    ...
    for (int s = 0; s < no_of_commands4; s++)
    {
        ...
        {
            strcpy(input_filename, commandsarray4[s + 1]);
            s++;
        }
        else if (strcmp(commandsarray4[s], ">") == 0)
        {
            strcpy(output_filename, commandsarray4[s + 1]);
            s++;
        }
        else
        {
    ...
    int output_file_descriptor = open(output_filename, O_WRONLY | O_CREAT | O_TRUNC, 0644);
