...
void pastevents_execute(char *position, struct list *head, char *homedirshell, char *prevdir, struct log *top);
...
void foreground(char *commandsarray[1024], char *homedirofshell, int no_of_commands, char *prevdir, char *set_of_commands2, struct list *head, struct log *top);
...
int appendfn(char *setofcommands, char *homedirofshell, char *prevdir, struct list *head, struct log *top, struct log *acti)
{
    ...
    int file_descriptor = open(filename, O_WRONLY | O_CREAT | O_APPEND, 0644);
    ...
    if (pid == 0)
    {
        ...
        {
            pastevents_execute(command[2], head, homedirofshell, prevdir, top);
        }
